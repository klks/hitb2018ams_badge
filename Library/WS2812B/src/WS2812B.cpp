...
if (!begun)
{
  SPI_1.setClockDivider(SPI_CLOCK_DIV16);// need bit rate of 400nS but closest we can do @ 72Mhz is 444ns (which is within spec)
  SPI_1.begin();
  begun = true;
}
}
...
{
  SPI_1.dmaSendAsync(pixels,numBytes);// Start the DMA transfer of the current pixel buffer to the LEDs and return immediately.
